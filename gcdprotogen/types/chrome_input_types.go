...
type ChromeInputTouchPoint struct {
	...
	RotationAngle float `json:"rotationAngle,omitempty"` // Rotation angle (default: 0.0).
	Force float `json:"force,omitempty"` // Force (default: 1.0).
	Id float `json:"id,omitempty"` // Identifier used to track touch sources between events, must be unique within an event.
}
