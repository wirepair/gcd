...
type ChromeCSSStyleSheetOrigin string // possible values: user, user-agent, inspector, regular
 
 
// CSS rule collection for a single pseudo style.
type ChromeCSSPseudoIdMatches struct {
	PseudoId int `json:"pseudoId"` // Pseudo style identifier (see <code>enum PseudoId</code> in <code>RenderStyleConstants.h</code>).
...
// An array of media query expressions.
type ChromeCSSMediaQuery []*ChromeCSSMediaQueryExpression 
