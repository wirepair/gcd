...
type NetworkExemptedSetCookieWithReason struct {
	ExemptionReason string         `json:"exemptionReason"` // The reason the cookie was exempted. enum values: None, UserSetting, TPCDMetadata, TPCDDeprecationTrial, TopLevelTPCDDeprecationTrial, TPCDHeuristics, EnterprisePolicy, StorageAccess, TopLevelStorageAccess, Scheme
...
type NetworkAssociatedCookie struct {
	...
	ExemptionReason string         `json:"exemptionReason,omitempty"` // The reason the cookie should have been blocked by 3PCD but is exempted. A cookie could only have at most one exemption reason. enum values: None, UserSetting, TPCDMetadata, TPCDDeprecationTrial, TopLevelTPCDDeprecationTrial, TPCDHeuristics, EnterprisePolicy, StorageAccess, TopLevelStorageAccess, Scheme
}
...
type NetworkDataReceivedEvent struct {
	...
	Params struct {
		...
		Data              string  `json:"data,omitempty"`    // Data that was received. (Encoded as a base64 string when passed over JSON)
...
// Returns -  bufferedData - Data that has been buffered until streaming is enabled. (Encoded as a base64 string when passed over JSON)
func (c *Network) StreamResourceContentWithParams(ctx context.Context, v *NetworkStreamResourceContentParams) (string, error) {
	...
	return chromeData.Result.BufferedData, nil
}
...
// Returns -  bufferedData - Data that has been buffered until streaming is enabled. (Encoded as a base64 string when passed over JSON)
func (c *Network) StreamResourceContent(ctx context.Context, requestId string) (string, error) {
