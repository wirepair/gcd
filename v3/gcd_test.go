...
func TestTargetCrashed(t *testing.T) {
	...
	targetCrashedFn := func(targ *ChromeTarget, _ int64, payload []byte) {
		t.Logf("reason: %s\n", string(payload))
		close(doneCh)
	}
...
func TestEvents(t *testing.T) {
	...
	target.Subscribe("Console.messageAdded", func(target *ChromeTarget, _ int64, v []byte) {
...
func TestEvaluate(t *testing.T) {
	...
	doneCh := make(chan struct{}, 1)
	target.Subscribe("Runtime.executionContextCreated", func(target *ChromeTarget, _ int64, v []byte) {
		//target.Unsubscribe("Console.messageAdded")
...
func TestComplexReturn(t *testing.T) {
	...
	target.Subscribe("Page.loadEventFired", func(target *ChromeTarget, _ int64, payload []byte) {
		var ok bool
...
func TestNetworkIntercept(t *testing.T) {
	...
	target.Subscribe("Fetch.requestPaused", func(target *ChromeTarget, _ int64, payload []byte) {
		close(doneCh)
